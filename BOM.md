...
| 21 | Ejector spring | light compression, **Ø9 OD × 11 free, solid ≤ 4.5, ~0.5 N/mm** (CNC rev 3e — the old Ø9 × 15 goes solid in the 5.8 mm under the head): sits on the latch-bore floor around the closure screw head, compressed to 5.8 by the seated head; dropped in AFTER the consumer screw | 1 | $2 |
| 21b | **Latch cap** (rev 3e) | Ø8 × 10 steel cap, Ø10 × 2 rear flange, Ø6.1 × 8 bore over the HS-0730B plunger nose, Ø2 × 8 roll pin — the latch pin proper. **Lathe, Ø10 O1 drill rod, torch-hardened** (4140 pre-hard fallback). Printed prototype `cnc-design/stl/proto/proto_cap.stl` | 1 | $3 stock |
| 21c | **Return spring** (rev 3e) | **the solenoid's own front spring, cut to 10 mm free** (7.3 mm room at rest, 4.7 retracted); replace with Ø9 OD × 10, ~0.4 N/mm only if the factory wire is > 0.7 mm | 1 | — |
| 21d | **Mouth bushing** (rev 3e) | Ø10.3 ID × Ø14 OD × 10.55, **same O1 lathe session as the cap and head**, hardened; light press or Loctite 638 into the A1 boss, through the lid to its top face. Buy alternative: ANSI type-P press-fit drill bushing 13/32" (Ø10.32 ID, Ø15.9 OD) — needs `BOSS_D` 20 and `BTN2_Y` 14 in the model. Printed prototype `proto_bushing.stl` | 1 | $4 stock |
| 21e | **Cable head** (rev 3e) | Ø10 × ~22 hardened: 3 mm 45° cone, 3 mm land, 8.5 × Ø6.8 groove (square lower flank, 45° upper), Ø8 ferrule bored Ø4.2 for the swaged cable. Lathe, O1. Printed prototype `proto_cable_head.stl` (with a grip + cord hole) | 1 | $3 stock |
...
| 23e | Pedestal + nano-clamp + driver-card screws + inserts | M3 machine ×6 + M3 brass heat-set ×4 + **short M3 heat-set ×2** — pedestal pads 2 (into body pads), nano-clamp 2, driver card 2 (short inserts; **v0.8.2: the driver card now rides the pedestal-cart's -Y platform**, not the body — a tiny flyback loop with the solenoid the cart carries, and driver+solenoid+cart pull as one serviceable module) | 6 + 6 | $2 |
