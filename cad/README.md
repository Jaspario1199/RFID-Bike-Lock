...
blind hinge pins and their glued end plugs — D7), and card-edge board pockets.

## v0.8 — all-M3 heat-set fastening (owner request)

For a super-refined printed prototype, **every threaded joint is now an M3 machine screw
into an M3 brass heat-set insert** — no self-tapping-into-plastic anywhere. The conversion
replaced every Ø2.5 self-tap pilot with an INS3 pocket (Ø4.1×6.5), switched the bay screws
from M4 self-tap to M3 socket-cap into inserts, dropped the closure screw from M4 to a short
M3 heat-set (Ø4.0×L3.8, the pad is only 3.4 mm deep), and moved all cover parts to
machine-flat countersinks (ISO 10642, 90°) since they now thread into inserts rather than
self-tap. Two carve-outs, both forced by an external part and both flagged in the BOM:
- **Solenoid → M2.5** — the JF-0530B's own mounting tabs are drilled M2.5; an M3 screw
  physically won't pass, so this one joint keeps an M2.5 brass heat-set.
- **PN532 → M3 nylon** — the antenna loop wraps the whole board perimeter, so brass/steel at
  the 4 corner holes sits in the 13.56 MHz field and kills read range; the M3 standard is
  kept but the screws are nylon into printed bosses (no insert). All 4 corners, no safe subset.

Two geometry side-effects were handled and re-gated to 0: the spool-cover screws couldn't take
a Ø4.1 insert in the 2.5 mm-thin ring wall, so each of the 3 stations gained a small inboard
buttress that backs the insert without fouling the cover recess; and two vestigial features
(the nano-*sled* foot pilots and the deleted perf-rack's hatch cartridge screws) were removed.
Gate after conversion: **0 violations / 292 checks · 0 clashes · sweep 0.00 · corridor clean.**
