...
| [`CNC_CASING.md`](CNC_CASING.md) | **The machined-casing redesign** — CNC-first architecture (tube-half chassis + inside-out bolted boxes, slide-and-lift install with two hooks + one guarded screw, no hinge, no inserts), DFM rulebook, decisions; model `cad/cnc_casing_cq.py`, renders `renders/cnc/` |
