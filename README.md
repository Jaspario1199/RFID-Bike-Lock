...
| [`CAD_METHOD.md`](CAD_METHOD.md) | **Portable method doc** — how to model and verify mechanical parts with an agent: the gate catalogue, the iteration loop, honesty rules, and an opening prompt for a new project |
...
| [`CAD_AUDIT.md`](CAD_AUDIT.md) | **Build audit of the CNC casing (rev 3d)** — what `--audit` checks beyond the geometry gates (wire/connector envelopes, nuts, plugs, thread engagement, pin insertion, machining numerics), every defect it found and what moved |
| [`ASSEMBLY_CNC.md`](ASSEMBLY_CNC.md) | **The step-by-step build manual for the current CNC casing (rev 3d)** — print settings, every screw, bench checkpoints, stage-by-stage assembly, troubleshooting |
